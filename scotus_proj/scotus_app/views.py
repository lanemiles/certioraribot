...
def index(request):
    ...
    cases_lst = sorted(list(cases), key=lambda x: (
        x.docketed_date(), x.term_year, x.case_number), reverse=True)[:500]
...
def cases_to_consider_for_cfr(request):
    cases = Case.objects.filter(
        consider_for_cfr=True, need_to_send_cfr_email=False)
    cases_lst = sorted(list(cases), key=lambda x: (
        x.docketed_date(), x.term_year, x.case_number), reverse=True)
    return render(request, 'scotus_app/index.html', {
...
def cases_with_cfr(request):
    ...
    for c in cases:
        try:
            ...
            continue
    cases_lst = sorted(list(cases_with_cfr), key=lambda x: (
        x.docketed_date(), x.term_year, x.case_number), reverse=True)
    return render(request, 'scotus_app/index.html', {
